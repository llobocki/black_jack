...
SUITE(Box) {
  ...
  TEST(card) {
    for (int i = 1; i < 5; ++i)
      for (int j = 2; j < 14; ++j) {
        ...
        CHECK_EQUAL(10, box.get_bet());
        CHECK_EQUAL(
            [](Card c) { return c.get_value() < 11 ? c.get_value() : 10; }(c),
            box.get_value());
  ...
  TEST(two_card) {
    for (int j = 2; j < 14; ++j) {
      ...
      CHECK_EQUAL(10, box.get_bet());
      CHECK_EQUAL(
          [](Card c) { return c.get_value() < 10 ? c.get_value() : 10; }(c)*2,
          box.get_value());
  ...
  TEST(black_jack) {
    ...
    for (int i = 1; i < 14; ++i) {
      ...
      CHECK_EQUAL(
          [](Card c) {
            return c.get_value() < 10 ? c.get_value() : 10;
          }(second) +
              11,
          box.get_value());
      CHECK_EQUAL(
          [](Card c) { return c.get_value() < 10 ? false : true; }(second),
          box.black_jack());
    }
  }
  ...
  TEST(sof_ace_many_aces) {
    ...
    CHECK_EQUAL(21, box.get_value());
    CHECK_EQUAL(false, box.soft_ace());
    CHECK_EQUAL(false, box.black_jack());
    box.card(Card(Card::Colour(1), Card::Value(1)));
    CHECK_EQUAL(22, box.get_value());
    CHECK_EQUAL(false, box.soft_ace());
  }
}
